...
def generate_block(transaction_info):
    txdata = subprocess.run(f"bitcoin-cli decoderawtransaction {transaction_info}",
        shell=True,
        stderr=subprocess.PIPE,
        stdout=subprocess.PIPE,
        encoding="utf-8").stdout
    
    subprocess.run(f"bitcoin-cli generateblock {wallet.address.mainnet.pubaddr1} {txdata}",
                   shell=True)
